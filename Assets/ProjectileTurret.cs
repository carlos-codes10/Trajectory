...
public class ProjectileTurret : MonoBehaviour
{
    ...
    void DrawTrajectory()
    {
        ...
        List<Vector3> pts = new List<Vector3>(maxPoints);
        pts.Add(startPos);

        Vector3 prev = startPos;

        for (float t = dt; t <= maxTime && pts.Count < maxPoints; t += dt)
        {
            Vector3 next = startPos + startVel * t + 0.5f * g * Mathf.Pow(t,2);

            Vector3 seg = next - prev;
            Vector3 segDir = seg.normalized;
            float segLen = seg.magnitude;
            if (segLen > 0f && Physics.Raycast(prev, segDir, out RaycastHit hit, segLen, targetLayer))
            {
                pts.Add(hit.point);
                break;
            }
